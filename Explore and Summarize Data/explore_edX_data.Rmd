...
ggplot(aes(abbrev_course_title,fill = LoE_DI),data = filter(edxdata,((LoE_DI!="") & (!is.nan(LoE_DI))))) +
...
### Plot Two
```{r,echo=FALSE,warning=FALSE}
ggplot(aes(x=abbrev_course_title,y=nevents),data=filter(edxdata,certified==1))+geom_jitter(aes(color=short_title),alpha=0.2,na.rm=TRUE)+ylim(0,30000)+geom_boxplot()+coord_flip()
```
...
ggplot(aes(x = nevents/10000,y = grade,color=certified),
       data = filter(edxdata,explored == 1)) +
  geom_point(size=0.5) + facet_wrap(~abbrev_course_title) +xlim(0,2)
```
